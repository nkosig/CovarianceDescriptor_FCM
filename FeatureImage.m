function [ FeatureIM ] = FeatureImage( RGBimage )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
%  FeatureIM - Output feature image (W x H x d)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    FeatureIM = zeros(h,w,9);               % Feature image template  (H x W x d matrix)

    for i=1:h
        for j=1:w
            FeatureIM(i,:,1) = i*ones(1,w);         % Pixel x location
            FeatureIM(:,j,2) = j*ones(h,1);         % Pixel y location
        end
    end

    FeatureIM(:,:,3:5) = im2double(RGB);            % double RGB pixel values
    ...
    FeatureIM(:,:,6) = abs(conv2(I,xdev1,'same'));    % Absolute of first derivative w.r.t x |dI/dx|
    FeatureIM(:,:,7) = abs(conv2(I,ydev1,'same'));    % Absolute of first derivative w.r.t y |dI/dy|
    FeatureIM(:,:,8) = abs(conv2(I,xdev2,'same'));    % Absolute of second derivative w.r.t x |d^2I/dx^2|
    FeatureIM(:,:,9) = abs(conv2(I,ydev2,'same'));    % Absolute of second derivative w.r.t y |d^2I/dy^2|

    FeatureIM = permute(FeatureIM,[2 1 3]); % permute F from H x W x d to W x H x d
end
